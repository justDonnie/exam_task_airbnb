...
alter type family_status rename to FamilyStatus;
create table owners(
...
select o.first_name,h.* from owners o inner join houses h on o.id = h.owner_id  order by length(o.first_name) desc limit 1;--1
select true from houses where price between '1500'and '2000' limit 1;--2
select * from houses where address_id = '5' or address_id = '6' or address_id = '7' or address_id = '8' or address_id ='9' or address_id = '10';--3
select houses, owners.first_name||' '|| owners.last_name as owners,customers.first_name||' '||customers.last_name as customers,agencies.name from rent_info inner join houses on rent_info.house_id = houses.id inner join owners on rent_info.owner_id = owners.id inner join customers on rent_info.customer_id = customers.id inner join agencies on rent_info.agency_id = agencies.id;--4
select customers from customers where date_of_birth  < '01/01/1999' limit 15 offset 10;--5
select houses.rating,houses.house_type,owners from houses inner join owners on houses.owner_id = owners.id order by houses.rating desc;--6
select count(house_type),sum(price) from houses where house_type = 'Apartment';--7
...
select owners.first_name||' '|| owners.last_name as owners,addresses from houses inner join owners on houses.owner_id = owners.id inner join addresses on houses.address_id = addresses.id where houses.furniture is true;--9
select max(rating),min(rating),(select rating/2)from houses group by houses --??10
select customers.nationality,count(customers.nationality) from customers group by nationality;--11
select house_type, max(rating)from houses group by houses.house_type;--?? 12
select customers,houses from rent_info full join customers on rent_info.customer_id = customers.id full join houses on rent_info.house_id = houses.id  ;--13
select avg(price)from houses;--14
select owners.first_name||' '|| owners.last_name as owners,customers.first_name||' '||customers.last_name as customers from rent_info inner join owners on rent_info.owner_id = owners.id inner join customers on rent_info.customer_id = customers.id where owners.first_name like 'A%';--15
select count(houses),owners from houses inner join owners on houses.owner_id = owners.id group by owners ;--16
select customers.first_name||' '||customers.last_name as customers ,nationality from customers where marital_status = 'Married';--17
select owners.first_name||' '|| owners.last_name as owners,count(houses.room),addresses from houses inner join owners on houses.owner_id = owners.id inner join addresses on houses.address_id = addresses.id group by addresses,houses.room,owners.first_name||' '|| owners.last_name;--18??
select houses,addresses from addresses inner join houses on addresses.id = houses.address_id where city = 'Bishkek'; --19
select customers.first_name||' '||customers.last_name as customers,customers.gender from customers group by customers.first_name||' '||customers.last_name,gender;--20.1
select owners.first_name||' '||owners.last_name as owners,owners.gender from owners group by owners.first_name||' '||owners.last_name,gender;--20.2
select houses from rent_info inner join houses on rent_info.house_id = houses.id between rent_info.check_in and rent_info.check_out;--21??
select houses.price,owners.first_name from houses inner join owners on houses.owner_id = owners.id order by price desc limit 1;--22
select agencies,addresses.region from addresses inner join agencies on addresses.id = agencies.address_id where region = region;--23??
select max(rating) from houses group by rating order by rating desc limit 5;--24
select addresses,houses,avg(date_part('2023',age(current_date,date_of_birth))) as average_age from owners group by addresses, houses;
